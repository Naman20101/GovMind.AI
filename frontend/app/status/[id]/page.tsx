...
import { Clock, CheckCircle, XCircle, AlertTriangle, Users, Lock, Loader2, RefreshCw, ArrowLeft } from 'lucide-react'
...
const statusColor: Record<string, string> = {
  PENDING: 'text-[#F39C12]', APPROVED: 'text-[#27AE60]',
  FLAGGED: 'text-[#E74C3C]', REJECTED: 'text-[#E74C3C]', HUMAN_REVIEW: 'text-blue-600',
}
...
export default function StatusPage() {
  ...
  const load = async () => {
    setLoading(true)
    try { setPermit(await getPermit(id)) }
  ...
  const requestReview = async () => {
    ...
    try {
      setPermit(await reviewPermit(id, { decision: 'HUMAN_REVIEW', reason: 'Requested by applicant for manual review', reviewed_by: 'applicant' }))
      setReviewed(true)
    } catch { setError('Could not request review.') }
  ...
  if (error || !permit) return (
    <div className="max-w-2xl mx-auto px-6 py-16 text-center">
      ...
      <Link href="/apply" className="bg-[#1B4F72] text-white px-6 py-2.5 rounded-xl font-medium inline-flex hover:bg-[#154360] transition-all">Apply for a Permit</Link>
    </div>
  )

  return (
    <div className="max-w-2xl mx-auto px-6 py-12 space-y-6">
      <Link href="/" className="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-[#1B4F72] transition-colors">
      ...
      <div className="card animate-fade-up">
        <div className="flex items-start justify-between mb-4">
          <div>
            <span className="text-xs text-gray-400 font-mono">APP-{permit.id.slice(0, 8).toUpperCase()}</span>
            <h1 className="font-serif text-2xl text-[#1B4F72] mt-1">{permit.business_name}</h1>
        ...
        <div className="mt-4 pt-4 border-t border-gray-100 flex items-center gap-4 text-xs text-gray-400">
          <span className="flex items-center gap-1"><Clock className="w-3 h-3" />Submitted {new Date(permit.submitted_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</span>
        </div>
      </div>
      ...
      {permit.ai_decision && (
        <div className="card animate-fade-up delay-100">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className={`badge ${permit.ai_decision === 'APPROVED' ? 'badge-approved' : permit.ai_decision === 'FLAGGED' ? 'badge-flagged' : 'badge-pending'}`}>
              {permit.ai_decision}
            </span>
          </div>
          {permit.ai_confidence && (
            <div className="mb-4">
              <div className="flex justify-between text-xs text-gray-500 mb-1.5">
                ...
                <span className="font-semibold text-[#1B4F72]">{permit.ai_confidence}%</span>
              </div>
              <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                <div className="h-full bg-[#1B4F72] rounded-full transition-all duration-700" style={{ width: `${permit.ai_confidence}%` }} />
              </div>
            </div>
          )}
          {permit.ai_reason && (
            <p className="text-sm text-gray-600 leading-relaxed bg-gray-50 rounded-xl p-4 mb-4">{permit.ai_reason}</p>
          )}
          {reviewed ? (
            <div className="flex items-center gap-2 text-sm text-blue-600 bg-blue-50 px-4 py-3 rounded-xl">
              <CheckCircle className="w-4 h-4" />Human review requested. An officer will contact you shortly.
            </div>
          ) : (
            <button onClick={requestReview} disabled={reviewLoading || permit.status === 'HUMAN_REVIEW'}
              ...
              {reviewLoading ? <><Loader2 className="w-4 h-4 animate-spin" />Requesting...</> : <><Users className="w-4 h-4" />Request Human Review</>}
            </button>
          )}
        </div>
      )}
      ...
      <div className="animate-fade-up delay-300">
        <AuditTraceViewer trace={permit.audit_trace} applicationId={permit.id} />
      </div>

      <button onClick={load} className="w-full flex items-center justify-center gap-2 text-sm text-gray-400 hover:text-[#1B4F72] py-3 transition-colors">
