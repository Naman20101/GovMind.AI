...
import { ArrowLeft, Camera, Upload, ArrowRight, AlertCircle, Loader2, CheckCircle, Shield, Users, Info, X } from 'lucide-react';

const translations = {
  English: {
    title: "Apply for a Permit",
    subtitle: "Get an AI-powered decision in minutes",
    guestWarning: "You're applying as a guest. Sign in or create an account to save your applications and track them later.",
  ...
  Arabic: {
    ...
    guestWarning: "أنت تتقدم كضيف. سجل الدخول أو أنشئ حسابًا لحفظ طلباتك وتتبعها لاحقًا.",
  ...
  Hindi: {
    ...
    guestWarning: "आप अतिथि के रूप में आवेदन कर रहे हैं। आवेदनों को सहेजने और ट्रैक करने के लिए साइन इन करें या अकाउंट बनाएं।",
...
export default function ApplyPage() {
  ...
  const openCamera = () => {
    navigator.mediaDevices.getUserMedia({ video: true })
      .then(() => alert("Camera opened — point at your ID document (demo mode)"))
        ...
        <div className="bg-white rounded-3xl shadow p-8 md:p-12">
          ...
          <div className="space-y-8">
            <div>
              ...
              <input type="text" className="w-full px-6 py-4 border border-gray-300 rounded-2xl" placeholder="e.g. Acme Corporation" />
            </div>

            <div>
              <label className="block text-sm font-medium mb-2">{t.permitType}</label>
              <select className="w-full px-6 py-4 border border-gray-300 rounded-2xl">
            ...
            <div>
              ...
              <input type="text" className="w-full px-6 py-4 border border-gray-300 rounded-2xl" placeholder="Full legal name" />
            </div>
            ...
            <button 
              onClick={openCamera}
              className="w-full flex items-center justify-center gap-3 bg-[#1B4F72] text-white py-4 rounded-2xl font-semibold hover:bg-[#0F3A5A]"
            >
          ...
          <button className="mt-12 w-full bg-[#F39C12] text-white py-5 rounded-2xl font-semibold text-lg hover:bg-[#E67E22]">
            {t.nextBtn} <ArrowRight className="inline ml-2" />
          </button>
        </div>
      </div>
    </div>
