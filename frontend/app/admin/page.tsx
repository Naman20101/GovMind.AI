...
export default function AdminPage() {
  ...
  const load = useCallback(async (isAutoRetry = false) => {
    ...
    try {
      ...
      if (res.status === 202) {
        setLoadingStep('Server waking up... please wait (~60 seconds)')
        setServerStatus('waking')
      ...
      setServerStatus('waking')
      setLoadingStep('Connecting... retrying...')
      retryRef.current = setTimeout(() => load(true), 5000)
    }
  }, [filter])

  useEffect(() => {
    if (adminAuth) { setLoading(true); load() }
    ...
    return (
      <div className="min-h-screen bg-[#F4F6F9] flex items-center justify-center px-6">
        <div className="bg-white rounded-2xl shadow-card p-8 w-full max-w-sm">
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <div className="relative">
                <input type={showPass ? 'text' : 'password'} className="input pr-16"
          ...
          <p className="text-center text-xs text-gray-400 mt-4">🔒 Restricted to authorized personnel only</p>
        </div>
      </div>
    )
  }
      ...
      <div className="mb-8 flex items-start justify-between">
        <div>
          ...
          <div className="flex items-center gap-3 mt-1">
            ...
            {serverStatus === 'online' && <span className="flex items-center gap-1 text-xs text-green-600"><Wifi className="w-3 h-3" />Online</span>}
            {serverStatus === 'waking' && <span className="flex items-center gap-1 text-xs text-amber-600"><Loader2 className="w-3 h-3 animate-spin" />Waking up...</span>}
            {serverStatus === 'offline' && <span className="flex items-center gap-1 text-xs text-red-500"><WifiOff className="w-3 h-3" />Reconnecting...</span>}
          </div>
          {lastRefresh && <p className="text-xs text-gray-400 mt-1">Last updated: {lastRefresh.toLocaleTimeString()}</p>}
        </div>
      ...
      {loading && loadingStep && (
        <div className="flex items-center gap-3 bg-blue-50 border border-blue-200 text-blue-700 px-5 py-4 rounded-xl mb-6">
          ...
          <div>
            ...
            <p className="text-xs mt-0.5 text-blue-600">Render free tier sleeps after inactivity. First load takes 30-60 seconds.</p>
          </div>
        </div>
      )}
      ...
      <div className="flex items-center gap-3 mb-5">
        <div className="relative">
          <select value={filter} onChange={e => setFilter(e.target.value)}
            className="appearance-none bg-white border border-gray-200 rounded-xl px-4 py-2.5 pr-8 text-sm font-medium text-gray-700 focus:outline-none focus:ring-2 focus:ring-[#1B4F72]/20 cursor-pointer">
        ...
        <button
          onClick={() => { if (retryRef.current) clearTimeout(retryRef.current); setLoading(true); setServerStatus('waking'); setLoadingStep('Refreshing...'); load() }}
          disabled={loading}
        ...
        <span className="text-xs text-gray-400 ml-auto">{filteredPermits.length} applications</span>
      </div>
        ...
        {loading && permits.length === 0 ? (
          <div className="flex items-center justify-center py-24">
            <div className="text-center max-w-sm">
              <div className="w-16 h-16 rounded-full border-4 border-[#1B4F72]/20 border-t-[#1B4F72] animate-spin mx-auto mb-4" />
              <p className="text-[#1B4F72] font-semibold text-lg mb-2">Loading Applications</p>
              <p className="text-gray-500 text-sm">{loadingStep || 'Connecting to server...'}</p>
              <p className="text-gray-400 text-xs mt-3">May take up to 60 seconds on first load</p>
            </div>
          </div>
          ...
          <div className="text-center py-20 text-gray-400">
            ...
            <p className="text-sm mt-1 mb-4">{loading ? 'Still loading...' : 'Try refreshing'}</p>
            {!loading && (
              <button onClick={() => { setLoading(true); setServerStatus('waking'); setLoadingStep('Retrying...'); load() }}
                className="text-sm text-[#1B4F72] underline">Retry</button>
            )}
          </div>
        ) : (
                ...
                {filteredPermits.map((p, i) => (
                  <tr key={p.id} className={`border-b border-gray-50 hover:bg-gray-50/50 transition-colors ${i % 2 === 0 ? '' : 'bg-gray-50/20'}`}>
                    <td className="px-5 py-4">
                      <span className="font-mono text-xs text-gray-400 bg-gray-100 px-2 py-1 rounded-lg">{p.id.slice(0,8)}...</span>
                    </td>
                    ...
                    <td className="px-5 py-4">
                      {p.ai_decision ? <StatusBadge status={p.ai_decision} /> : <span className="text-gray-300 text-sm">—</span>}
                    </td>
                    <td className="px-5 py-4">
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto animate-fade-up">
            ...
            <div className="p-6 space-y-5">
              {modal.fullData && (
                <AuditTraceViewer trace={modal.fullData.audit_trace} applicationId={modal.permit.id} />
              )}
              <div>
